...
dataset_path = './demo_images/season_TM01/' # fixed path
...
for dataset_root, dataset_dirs, dataset_files in os.walk(os.path.join(dataset_path), topdown=True, followlinks=True):
    ...
    if 'FORLAB' in id or 'FFHQ' in id:
        ...
        files = sorted([os.path.splitext(file)[0] for file in dataset_files])[:40000]
    else:
        # files = sorted([file.replace('.jpg', '').replace('.png', '') for file in dataset_files])
        files = sorted([os.path.splitext(file)[0] for file in dataset_files])
    print(len(files))
...
for dataset in [dataset for dataset in datasets if (dataset['shared'] == 'Telegram')]:
    ...
    train_set_post, val_set_post, test_set_post = random_split(files_post, [0.7, 0.15, 0.15]) # randomly split the post-social files into train/val/test sets
    # missing generator for reproducible results 
    ...
    print(len(train_set_pre)+len(train_set_post), len(val_set_pre)+len(val_set_post), len(test_set_pre)+len(test_set_post), ':', 
    len(train_set_pre)+len(train_set_post)+len(val_set_pre)+len(val_set_post)+len(test_set_pre)+len(test_set_post))
    #print(val_set)
    #print(test_set)
...
with open("split_prova.json", "w") as f:
    json.dump({'train': sorted(train_set), 'val': sorted(val_set), 'test': sorted(test_set)}, f, indent=2)