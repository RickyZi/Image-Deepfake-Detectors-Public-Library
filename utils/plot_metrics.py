...
def find_json(folder: Path) -> Path | None:
    """Return the metrics.json found in folder"""
...
def load_ref_scores(RESULTS_ROOT, BASELINE_SUBDIR, social='', ref_label: str = REF_LABEL) -> dict | None:
    """
    Load metrics from the reference folder (results/pretrained/dataset/).
    ...
    if not RESULTS_ROOT.exists():
        ...
        return None
    if social:
        ref_dir = Path(f"./PROVA/pretrained_social/{social}")
        dataset_label = str(social)
    else:
        # ref_dir = RESULTS_ROOT / REF_FOLDER
        ref_dir = Path("./PROVA/pretrained/dataset")
    ...
    for subdir in [BASELINE_SUBDIR]:
        ...
        if model_dir.is_dir():
            ...
            if json_path:
                    scores = load_metrics(json_path)
                    # breakpoint()
                    if scores:
                        display_label = str(model_dir).split('/')[-1] + '_' + dataset_label
                        print(f"  [ref] {display_label:<30s}  F1={scores['F1']:.4f}  (from {subdir})")
...
def collect_results(RESULTS_ROOT, subdir_name: str) -> dict[str, dict]:
    ...
    for dataset_dir in sorted(RESULTS_ROOT.iterdir()):
        ...
        print(f"  [ok] {label:<30s}  F1={metrics['F1']:.4f}")
...
def main():
    ...
    if args.social:
        out_dir = Path(f"./PROVA/metric_tables/social/pretrained_{args.social}")
        
        # out_dir = Path(f"./results/metric_tables/social/baseline/")
        OUTPUT_DIR= out_dir
        # out_dir.mkdir(parents=True, exist_ok=True)
        results = Path(f"./PROVA/pretrained_{args.social}/")
        # results = Path(f"./results/pretrained_social") # for baselines on social split
        # results = Path(f"./results/{args.social}")
        RESULTS_ROOT = results
    else:
        RESULTS_ROOT = Path("./PROVA/pretrained")
        OUTPUT_DIR   = Path("./PROVA/metric_tables")
