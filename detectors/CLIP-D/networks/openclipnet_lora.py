...
class OpenClipLinearLoRA(nn.Module):
    def __init__(self, num_classes=1, pretrain='clipL14commonpool', normalize=True,     next_to_last=True, lora_r=4, lora_alpha=1, lora_targets=('q','v')):
        super().__init__()
        backbone = open_clip.create_model(*dict_pretrain[pretrain][:2]
                                           if len(dict_pretrain[pretrain]) == 2 else ...)
        ...
        # inject LoRA into each ResidualAttentionBlock's attention
        for block in backbone.visual.transformer.resblocks:
            block.attn = PlainMultiheadAttentionLoRA(
                block.attn, r=lora_r, lora_alpha=lora_alpha, enable_lora=lora_targets
            )  # only LoRA A/B params inside here get requires_grad=True

        self.backbone = backbone   # registered normally now -> shows up in state_dict / .to()
        self.fc = ChannelLinear(self.num_features, num_classes)
        torch.nn.init.normal_(self.fc.weight.data, 0.0, 0.02)

    def forward_features(self, x):
        # NO torch.no_grad() here anymore — LoRA params need gradients
        features = self.backbone.encode_image(x, normalize=True)
        return features
