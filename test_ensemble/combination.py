...
def evaluate_rules(records, rules, threshold=0.5):
    ...
    for rule_name, rule_fn in rules.items():
        ...
        results[rule_name] = {
            'TPR': float(accuracy_score(labels[fake_mask], preds[fake_mask])) if fake_mask.sum() else 0.0,
            'TNR': float(accuracy_score(labels[real_mask], preds[real_mask])) if real_mask.sum() else 0.0,
            'Acc': float(accuracy_score(labels, preds)),
            'Balanced Acc': float(balanced_accuracy_score(labels, preds)),
            'F1': float(f1_score(labels, preds, labels=[0, 1], zero_division=0.0)),
